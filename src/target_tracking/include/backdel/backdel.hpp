...
namespace DeepSort {
    ...
    class DeepSortData {
    private:
        std::map<int, std::shared_ptr<Tracker>> trackers_; // track_id（唯一身份id，非车辆id） -> Tracker（对应到了跟踪器）
        std::map<TrackID, int> vehicle_armor_map_;  // 车辆类型ID -> 装甲板数字 ====================================（带修改）
        int next_self_id_ = 0; //本身固有的身份id
        std::mutex mutex_;
