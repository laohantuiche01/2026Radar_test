...
namespace auto_aim {
        ...
        if (!armors_.empty() && classifier != nullptr) {
            ...
            std::for_each(
                std::execution::par, armors_.begin(), armors_.end(), [this, &input](Armor &armor) {
                    ...
                    if (armor.number_img.cols < 0 || armor.number_img.rows < 0) return;
    ...
    cv::Mat Detector::preprocessImage(const cv::Mat &rgb_img) noexcept {
        ...
        cv::inRange(hsv, cv::Scalar(light_params.min_h, light_params.min_s, light_params.min_v),
                    cv::Scalar(light_params.max_h, light_params.max_s, light_params.max_v), mask);
        // cv::Mat kernel_ = cv::getStructuringElement(cv::MORPH_CROSS, cv::Size(3, 3));
        // cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2, 5));
        // erode(mask, mask, kernel_);
        // dilate(mask, mask, kernel, cv::Point(-1,-1), 2);

        cv::Mat kernel_vertical = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2, 5));
        cv::Mat kernel_horizontal = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2, 2));
        cv::Mat kernel_vertical_2 = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(1, 5));

        cv::Mat vertical_mat, horizontal_mat;
        cv::dilate(mask, vertical_mat, kernel_vertical, cv::Point(-1, -1), 1);
        cv::erode(vertical_mat, mask, kernel_horizontal);
        // cv::dilate(vertical_mat, horizontal_mat, kernel_vertical_2);
    ...
    bool Detector::isLight(const Light &light) noexcept {
        ...
        // bool length_ok = light_params.min_length < light.length && light.length < light_params.max_length;

        bool is_light = ratio_ok && angle_ok; //&& length_ok;

        // Fill in debug information
        DebugLight light_data;
        light_data.center_x = light.center.x;
        light_data.ratio = ratio;
        light_data.angle = light.tilt_angle;
        light_data.is_light = is_light;
        this->debug_lights.emplace_back(light_data);
    ...
    ArmorType Detector::isArmor(const Light &light_1, const Light &light_2) noexcept {
        ...
        //NOTE 确定灯条夹角是否符合要求
        float relative_angle = std::abs(light_1.tilt_angle - light_2.tilt_angle);
        bool relative_angle_ok = relative_angle < armor_params.max_relative_angle;

        bool is_armor = light_ratio_ok && center_distance_ok && angle_ok && relative_angle_ok;

        // Judge armor type
        ArmorType type;
        if (is_armor) {
            type = center_distance > armor_params.min_large_center_distance
                       ? ArmorType::LARGE
                       : ArmorType::SMALL;
        } else {
        ...
        this->debug_armors.emplace_back(armor_data);
